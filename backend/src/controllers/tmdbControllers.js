...
export const fetchVideosById = async (req, res, next) => {
  ...
  try {
    const response = await fetch(
      `${baseUrl}${id}/videos?language=en-US`, options
    );
    const data = await response.json();
    if (!response.ok) throw new Error(data.status_message);
...
export const fetchNowPlaying = async (req, res, next) => {
  try {
    const response = await fetch(
      `${baseUrl}now_playing?language=en-US&page=1`, options
    );
    const data = await response.json();
    if (!response.ok) throw new Error(data.status_message);
    res.status(200).json(data.results);
...
export const fetchDetailsById = async (req, res, next) => {
  ...
  try {
    const response = await fetch(
      `${baseUrl}${id}?language=en-US`, options
    );
    const data = await response.json();
    if (!response.ok) throw new Error(data.status_message);
...
export const fetchLogoById = async (req, res, next) => {
  const id = parseInt(req.params.id);

  try {
    const response = await fetch(
      `${baseUrl}${id}/images`, options
    );
    const data = await response.json();
    if (!response.ok) throw new Error(data.status_message);
